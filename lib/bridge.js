...
const OPERATIONS = {
    ...
    $map: (obj, field, map) => {
        if (typeof field !== 'string') {
            throw new InvalidOperationError('Invalid field key for $move. Expected string, got ' + typeof field)
        }
        if (typeof map !== 'object') {
            throw new InvalidOperationError('Invalid map key for $move. Expected object, got ' + typeof map)
        }
        ...
        for (let i = 0; i < segs.length; ++i) {
            ...
            if (seg === '$' && Array.isArray(_obj)) {
                for (let j = _obj.length - 1; j >= 0; --j) {
                    _obj[j] = OPERATIONS.$map(_obj[j], segs.slice(i + 1, segs.length).join('.'), map)
                }
     ...
     * @throws {InvalidOperationError}
     */
    $wrap: (obj, field, wrapper) => {
        if (typeof field !== 'string') {
            throw new InvalidOperationError('Invalid value for $wrap. Expected string, got ' + typeof field)
        }
        if (typeof wrapper !== 'string' && !Array.isArray(wrapper)) {
            throw new InvalidOperationError('Invalid value for $wrap. Expected string or array, got ' + typeof field)
        }
        if (typeof obj !== 'object') {
            return obj
        }
        ...
        for (let i = 0; i < segs.length; ++i) {
            ...
            if (seg === '$' && Array.isArray(_obj)) {
                for (let j = _obj.length - 1; j >= 0; --j) {
                    _obj[j] = OPERATIONS.$wrap(_obj[j], segs.slice(i + 1, segs.length).join('.'), wrapper)
                }
