...
class Gap {
    ...
    constructor({
        ...
        let _procedures = (bridge === null) ? {} : bridge.procedures
        this._operations = {
            request: {
                body: new Operations.Set(request.body, _procedures),
                headers: new Operations.Set(request.headers, _procedures),
                query: new Operations.Set(request.query, _procedures),
                params: new Operations.Set(request.params, _procedures)
            },
            response: {
                body: new Operations.Set(response.body, _procedures),
                headers: new Operations.Set(response.headers, _procedures)
            },
            error: {
                body: new Operations.Set(error.body, _procedures),
                headers: new Operations.Set(error.headers, _procedures)
            }
        }
	...
	/**
	 * Reset the stored procedures used by this gap's operation sets to the given procedure set
	 * 
	 * @param {object} procedures the new procedure set (format { [procedureName]: <OperationSet> })
	 */
	resetProcedures(procedures) {
        this._operations.request.body.resetProcedures(procedures)
        this._operations.request.headers.resetProcedures(procedures)
        this._operations.request.query.resetProcedures(procedures)
        this._operations.request.params.resetProcedures(procedures)
        this._operations.response.body.resetProcedures(procedures)
        this._operations.response.headers.resetProcedures(procedures)
        this._operations.error.body.resetProcedures(procedures)
        this._operations.error.headers.resetProcedures(procedures)
	}
