...
class Bridge {
    ...
    setProcedures(procName, op) {
        ...
        Object.keys(newProcs).forEach(proc => {
            this.procedures[proc] = op
        })

        // Update all of the bridge's gap procedures
        let itr = this.gapMap.values()
        let gap
        while (gap = itr.next().value) {
            gap.resetProcedures(this.procedures)
        }
    }
}
