...
class OperationSet {
    ...
    constructor(ops = [], models = {}) {
        ...
        OperationSet.verify(this.operations);
    }
                ...
                switch (op) {
                    case '$func':
                        if (typeof _value === 'string' || Array.isArray(_value)) {
                            _value = { '': _value } // normalize format
                        }
                        obj = _value(obj)
                        Object.keys(_value).forEach(field => {
                            let fieldArr = field.split('.').filter(s => s !== '')
                            obj = OperationSet.applyOperationAt(_value, obj, fieldArr, [_value[field]])
                        })
                        break
                    case '$set':
                        Object.keys(_value).forEach(field => {
                            ...
                            obj = OperationSet.applyOperationAt(module.exports.$set, obj, fieldArr, [lastKey, _value[field]])
                        })
                        break
                    case '$unset':
                        ...
                        _value.forEach(field => {
                            let fieldArr = field.split('.').filter(s => s !== '')
                            let lastKey = fieldArr.pop()
                            obj = OperationSet.applyOperationAt(module.exports.$unset, obj, fieldArr, [lastKey])
                        })
                        break
                    case '$move': // TODO
                    case '$copy':
                        Object.keys(_value).forEach(field => {
                            let commonFieldArr = []
                            let toFieldArr = _value[field].split('.').filter(s => s !== '')
                            let fromFieldArr = field.split('.').filter(s => s !== '')
                            ...
                            obj = OperationSet.applyOperationAt(module.exports[op], obj, commonFieldArr, [fromFieldArr, toFieldArr])
                        })
                        break
                    case '$wrap':
                        if (typeof _value === 'string' || Array.isArray(_value)) {
                        ...
                        Object.keys(_value).forEach(field => {
                            let fieldArr = field.split('.').filter(s => s !== '')
                            obj = OperationSet.applyOperationAt(module.exports.$wrap, obj, fieldArr, [_value[field]])
                        })
                        break
                    case '$cast':
                        Object.keys(_value).forEach(field => {
                            let fieldArr = field.split('.').filter(s => s !== '')
                            obj = OperationSet.applyOperationAt(module.exports.$cast, obj, fieldArr, [_value[field]])
                        })
                        break
                    case '$map':
                        Object.keys(_value).forEach(field => {
                            let fieldArr = field.split('.').filter(s => s !== '')
                            obj = OperationSet.applyOperationAt(module.exports.$map, obj, fieldArr, [_value[field]])
                        })
                        break
                    case '$models': // TODO
                        // handle alias of { $model: 'model' } => { $model: { '.': ['model'] } }
                        if (typeof _value === 'string') _value = { '': _value }

                        Object.keys(_value).forEach(_path => {
                            let _models = _value[_path]
                            if (!Array.isArray(_models)) _models = [_models]
                            _models.forEach(_model => {
                                if (!this.models[_model]) {
                                    throw new InvalidOperationError('No such model: ' + _model)
                                } else {
                                    obj = OperationSet.applySet(this.models[_model], obj, _path)
                                }
                            })
                        })
                        break
     ...
     * @throws {TypeError}
     */
    static applySet(opSet, obj, keys = '') {
    ...
    static verify(ops = {}) {
        if (Array.isArray(ops)) {
            return ops.forEach(OperationSet.verify)
        }
        ...
        for (let key in ops) {
            switch (key) {
                 ...
                 */
                case '$unset':
                    if (typeof ops[key] !== 'string' && (!Array.isArray(ops[key]) || ops[key].some(o => typeof o !== 'string'))) {
                 ...
                 */
                case '$wrap':
                    if (typeof ops[key] !== 'string' && typeof ops[key] !== 'object' && !Array.isArray(ops[key])) {
                 ...
                 */
                case '$models':
                    if (typeof ops[key] !== 'object' && typeof ops[key] !== 'string') {
                        throw new InvalidOperationError('Invalid value for $models. Expected string or object, got ' + typeof ops[key])
                    }
                    break
                default:
 ...
 * 
 * SPECIAL
 *  $models - list of stored bridge models to apply in this opset
