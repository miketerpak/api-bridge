                ...
                switch (op) {
                    ...
                    case '$procedures':
						...
						Object.keys(_value).forEach(_path => {
							let _tags = _value[_path]
							if (!Array.isArray(_tags)) _tags = [_tags]
							_tags.forEach(_tag => {
								if (!this.procedures[_tag]) {
									throw new InvalidOperationError('No such procedure: ' + _tag)
								} else {
									obj = OperationSet.applySet(this.procedures[_tag], obj, _path)
								}
							})
						})
                        break
